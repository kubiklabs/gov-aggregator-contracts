...
use super::types::DELEGATOR_FIELD;
...
/// Creates a message to register an Interchain Query to get delegate(undelegate?) for a delegator on a remote chain.
///
/// * **connection_id** is an IBC connection identifier between Neutron and remote chain;
/// * **delegator** is an address of an account on remote chain for which you want to get list of delegate transactions;
/// * **update_period** is used to say how often the query must be updated.
/// * **min_height** is used to set min height for query (by default = 0).
pub fn new_register_delegate_query_msg(
    connection_id: String,
    delegator: String,
    update_period: u64,
    min_height: Option<u64>,
    ...
    let mut query_data = vec![TransactionFilterItem {
        field: DELEGATOR_FIELD.to_string(),
        op: TransactionFilterOp::Eq,
        value: TransactionFilterValue::String(delegator),
    }];
    if let Some(min_height) = min_height {
        query_data.push(TransactionFilterItem {
            field: HEIGHT_FIELD.to_string(),
